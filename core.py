...
class HackingTool:
    ...
    def is_installed(self) -> bool:
        """Check if the tool is available — PATH binary or cloned directory.

        Nix-aware: tries multiple binary candidates derived from install
        commands so that system-installed tools (nixpkgs, Go, pip) are
        detected without needing a local git clone.
        """
        _SKIP = {".", "echo", "cd", "sudo", "doas", "bash", "sh",
        ...
        for ic in self.INSTALL_COMMANDS:
            ...
            elif re.search(r"pip[23]?\s+install", ic):
            ...
            # then check tools_dir for a real local clone
            elif "git clone" in ic:
                parts = ic.split()
                ...
                if (get_tools_dir() / clone_dir).exists():
                    return True
        ...
        for ic in (self.INSTALL_COMMANDS or []):
            if "git clone" in ic:
                ...
                if repo_urls:
                    ...
                    clone_path = get_tools_dir() / dirname
                    if clone_path.exists():
                        console.print(f"[cyan]→ git -C {clone_path} pull[/cyan]")
                        os.system(f"git -C {str(clone_path)!r} pull")
                        updated = True
    ...
    def _get_tool_dir(self) -> str | None:
        ...
        # 1. Check git clone target dir (always relative to tools_dir, never CWD)
        from config import get_tools_dir
        for ic in (self.INSTALL_COMMANDS or []):
            if "git clone" in ic:
                parts = ic.split()
                ...
                if repo_urls:
                    ...
                    if url_idx + 1 < len(parts) and not parts[url_idx + 1].startswith("-"):
                        dirname = parts[url_idx + 1]
                    candidate = get_tools_dir() / dirname
                    if candidate.exists():
